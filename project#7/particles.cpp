...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- wanghaox" };
...
const char *CL_FILE_NAME   = { "wanghaox.cl" };
const char *CL_BINARY_NAME = { "wanghaox.nv" };
...
Display( )
{
	...
	{
		char str[128];
		sprintf( str, "%6.2f MegaParticles/Sec", (float)NUM_PARTICLES/ElapsedTime/1000000. );
...
InitLists( )
{
	...
	glNewList( SphereList, GL_COMPILE );
		glColor3f( .9f, .9f, 0. );
		glPushMatrix( );
			glTranslatef( -300., -500., -200. );
			glutWireSphere( 300., 50., 50. );
		glPopMatrix( );
		glColor3f( 0., .9f, .9f );
		glPushMatrix( );
